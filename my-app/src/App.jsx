...
// Helper Functions (moved outside - don't depend on component state)
...
const getWeatherGradient = (weatherDesc, icon, temp) => {
  ...
  if (isNight) {
    ...
    if (desc.includes('snow')) {
      return 'linear-gradient(135deg, #373b44 0%, #4a5568 50%, #718096 100%)';
    }
    if (desc.includes('thunder') || desc.includes('storm')) {
      return 'linear-gradient(135deg, #141e30 0%, #243b55 50%, #1a1a2e 100%)';
    }
  ...
  if (desc.includes('clear') || desc.includes('sunny')) {
    if (temp > 85) {
      return 'linear-gradient(135deg, #f093fb 0%, #f5576c 50%, #ff6b6b 100%)';
    }
  ...
  if (desc.includes('rain') || desc.includes('drizzle')) {
    if (desc.includes('thunder') || desc.includes('storm')) {
      return 'linear-gradient(135deg, #2c3e50 0%, #34495e 50%, #596275 100%)';
    }
    return 'linear-gradient(135deg, #5f72bd 0%, #9b23ea 50%, #667eea 100%)';
  }
  ...
  if (desc.includes('cloud')) {
    ...
    return 'linear-gradient(135deg, #bdc3c7 0%, #8e9eab 50%, #7f8c9d 100%)';
  }
  ...
  if (desc.includes('fog') || desc.includes('mist') || desc.includes('haze')) {
    return 'linear-gradient(135deg, #bdc3c7 0%, #b8c6db 50%, #d8e2dc 100%)';
  }
  
  return 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)';
};

// Weather Animation Component (moved outside)
...
// MainPage Component (moved outside)
const MainPage = ({ 
  weatherInfo, 
  city, 
  setCity, 
...
// HourlyDetailPage Component (moved outside)
const HourlyDetailPage = ({ selectedHour, setCurrentPage, hourlyData, weatherInfo }) => {
...
// DayDetailPage Component (moved outside) - truncated for length, but includes full implementation
const DayDetailPage = ({ selectedDay, setCurrentPage }) => {
...
// FullHourlyPage Component (moved outside)
const FullHourlyPage = ({ setCurrentPage, hourlyData, setSelectedHour }) => {
  return (
    <div className="detail-page">
...
// FullDailyPage Component (moved outside)
const FullDailyPage = ({ setCurrentPage, dailyData, setSelectedDay }) => {
  return (
    <div className="day-detail-page">
      <button className="back-button" onClick={() => setCurrentPage('main')}>← Back to Overview</button>
  ...
  return (
    <>
      ...
      <div className="container">
        ...
        {!loading && !error && currentPage === 'main' && (
          <MainPage 
        ...
        {!loading && !error && currentPage === 'hourly-detail' && (
          <HourlyDetailPage 
        ...
        {!loading && !error && currentPage === 'day-detail' && (
          <DayDetailPage 
            selectedDay={selectedDay}
            setCurrentPage={setCurrentPage}
          />
        )}
        {!loading && !error && currentPage === 'hourly-full' && (
          <FullHourlyPage 
            setCurrentPage={setCurrentPage}
            hourlyData={hourlyData}
            setSelectedHour={setSelectedHour}
        ...
        {!loading && !error && currentPage === 'daily-full' && (
          <FullDailyPage 
            setCurrentPage={setCurrentPage}
            dailyData={dailyData}
            setSelectedDay={setSelectedDay}
